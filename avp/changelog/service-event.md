...
ACT="/jffs/scripts/avp/bin/avp-action"
...
run_action() {
  ...
  if [ ! -x "$ACT" ]; then
    json_err false 10 "dispatch" "avp-action not found" >"$tmp"
    write_last "$tmp"
    log "event=avp_webui_restart action=$action rc=10 msg=avp-action not found"
